...
public class AdminAboutUsController {
    ...
    public String toList() {
        return "admin/system/aboutus/list";
    }
    
    
    @RequestMapping("/toAdd.html")
    public String toAdd() {
        return "admin/system/aboutus/add";
    }
    ...
    public String toEdit(int id, Map<String, Object> map) {
        ...
        return "admin/system/aboutus/edit";
    }

    @ResponseBody
    @RequestMapping("/list.do")
    public ResultBean<List<Aboutus>> listAboutus(int pageindex,
                                                 @RequestParam(value = "pageSize", defaultValue = "15") int pageSize) {
        Pageable pageable = new PageRequest(pageindex, pageSize, null);
        List<Aboutus> list = aboutusService.findAll(pageable).getContent();
    ...
    public ResultBean<Integer> getTotal() {
        ...
        int total = (int) aboutusService.findAll(pageable).getTotalElements();
    ...
    public ResultBean<Boolean> del(int id) {
    	aboutusService.delById(id);
        return new ResultBean<>(true);
    }

    @RequestMapping(method = RequestMethod.POST, value = "/add.do")
    public void add(String companyDesc,
                    HttpServletRequest request,
                    HttpServletResponse response) throws Exception {
    	Aboutus aboutus = new Aboutus();
    	aboutus.setCompanyDesc(companyDesc);
        int id = aboutusService.create(aboutus);
        if (id <= 0) {
